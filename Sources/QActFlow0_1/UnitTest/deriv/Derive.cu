...
void integrate_func1(cuComplex* spec_old, cuComplex* spec_curr, cuComplex* spec_new, cuComplex* spec_nonl,
    ...
    if(i < Nxh && j < Ny){
        ...
        // u_{n+1} = u_{n}*exp(alpha * dt/2) + 1/6*exp(alpha*dt)*(a_n)
...
__global__ void integrate_func2(cuComplex* spec_old, cuComplex* spec_curr, cuComplex* spec_new, 
    ...
    if(i < Nxh && j < Ny){
        ...
        // u_{n+1} = u_{n}*exp(alpha * dt/2) + 1/6*exp(alpha*dt)*(a_n) + 1/6*exp(alpha*dt/2)*(b_n)
...
__global__ void integrate_func3(cuComplex* spec_old, cuComplex* spec_curr, cuComplex* spec_new, 
    ...
    if(i < Nxh && j < Ny){
        ...
        // u_{n+1} = u_{n}*exp(alpha * dt/2) + 1/6*exp(alpha*dt)*(a_n) + 1/3*exp(alpha*dt/2)*(b_n) 
...
int main(){
    ...
    coord(mesh);
    field_visual(w, "w.csv"); // w = cos(x)
    field_visual(r1, "r1.csv"); // r1 = sin(x)
    field_visual(alpha, "alpha.csv");

    // Fldset test
    FldSet<<<dimGrid,dimBlock>>>(alpha.phys, 0.1, mesh.Nx, mesh.Ny);
    field_visual(alpha, "alpha_set.csv");
    
    // S function calculation test
    S_func<<<dimGrid,dimBlock>>>(w.phys, r1.phys, S.phys, w.mesh->Nx, w.mesh->Ny);// test the callable of mesh in field
    cuda_error_func(cudaDeviceSynchronize());
    field_visual(S, "S.csv");

    // FldMul and FldAdd in-place test
    FldMul<<<dimGrid,dimBlock>>>(r1.phys, r1.phys, 1.f, r2.phys, r2.mesh->Nx, r2.mesh->Ny);
    FldMul<<<dimGrid,dimBlock>>>(w.phys, w.phys, 1.f, wcurr.phys, wcurr.mesh->Nx, wcurr.mesh->Ny);
    // test the in-place fld Add here(sin^2+cos^2 = r2)
    FldAdd<<<dimGrid,dimBlock>>>(1.0, wcurr.phys,1.0, r2.phys, r2.phys, r2.mesh->Nx, r2.mesh->Ny);
    cuda_error_func(cudaDeviceSynchronize());
    field_visual(r2,"r2.csv");

    // xDerivative and Fwd, Bwd transform test
    FwdTrans(w.mesh, w.phys, w.spec);
    xDeriv(w.spec, w.spec, w.mesh);
    BwdTrans(w.mesh, w.spec, w.phys);
    field_visual(w,"w_x.csv");// here we prove that in-place operation is permitted by derivative
