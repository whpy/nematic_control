...
void PhiinitD(float* phys, int Nx, int Ny, int BSZ, float dx, float dy){
    ...
    float y = j*dy;
    if (i<Nx && j<Ny){
        phys[index] = exp(-1*( (x-M_PI)*(x-M_PI)+(y-M_PI)*(y-M_PI) ));
    }
}
...
void wexactD(float* phys, int Nx, int Ny, int BSZ, float dx, float dy){
    ...
    float x = i*dx;
    float y = j*dy;
    float s = exp(-1*( (x-M_PI)*(x-M_PI)+(y-M_PI)*(y-M_PI) ));
    if (i<Nx && j<Ny){
        phys[index] = -4*s+4*s*(x-M_PI)*(x-M_PI) + 4*s*(y-M_PI)*(y-M_PI);
    }
}
...
void vexactD(float* phys, int Nx, int Ny, int BSZ, float dx, float dy){
    ...
    float s = exp(-1*( (x-M_PI)*(x-M_PI)+(y-M_PI)*(y-M_PI) ));
    if (i<Nx && j<Ny){
        phys[index] = -2*s*(x-M_PI);
    }
}

__global__
void NL1exact(float* phys, float lambda, float Pe, float cn, int Nx, int Ny, int BSZ, float dx, float dy){
    int i = blockIdx.x * BSZ + threadIdx.x;
    int j = blockIdx.y * BSZ + threadIdx.y;
    int index = i + j*Nx;
    float x = i*dx;
    ...
    float s = exp(-1*( (x-M_PI)*(x-M_PI)+(y-M_PI)*(y-M_PI) ));
    if (i<Nx && j<Ny){
        phys[index] = 1.f/Pe * s*(-8*(x-M_PI)*(y-M_PI)*lambda + 
        2*Pe*(2-4*M_PI*M_PI-x-2*x*x +y-2*y*y+4*M_PI*(x+y))*sin(x+y)-
        4*1/s * cos(x+y)*cn*cn );
    }
}

__global__
void Single1exact(float* phys, float lambda, float cn, int Nx, int Ny, int BSZ, float dx, float dy){
    int i = blockIdx.x * BSZ + threadIdx.x;
    int j = blockIdx.y * BSZ + threadIdx.y;
    int index = i + j*Nx;
    float x = i*dx;
    ...
    float s = exp(-1*( (x-M_PI)*(x-M_PI)+(y-M_PI)*(y-M_PI) ));
    if (i<Nx && j<Ny){
        phys[index] = cos(x+y)*(1+4*lambda+6*lambda*cn*cn);
    }
}

__global__
void Single2exact(float* phys, float lambda, float cn, int Nx, int Ny, int BSZ, float dx, float dy){
    int i = blockIdx.x * BSZ + threadIdx.x;
    int j = blockIdx.y * BSZ + threadIdx.y;
    int index = i + j*Nx;
    float x = i*dx;
    ...
    float s = exp(-1*( (x-M_PI)*(x-M_PI)+(y-M_PI)*(y-M_PI) ));
    if (i<Nx && j<Ny){
        phys[index] = sin(x+y)*(1.f+4.f*lambda+6*lambda*cn*cn);
    }
}

__global__
void crossexact(float* phys, float lambda, float cn, int Nx, int Ny, int BSZ, float dx, float dy){
    int i = blockIdx.x * BSZ + threadIdx.x;
    int j = blockIdx.y * BSZ + threadIdx.y;
    int index = i + j*Nx;
    float x = i*dx;
    ...
    float s = exp(-1*( (x-M_PI)*(x-M_PI)+(y-M_PI)*(y-M_PI) ));
    if (i<Nx && j<Ny){
        phys[index] = 10*cos(2*x+y)*sin(x+3*y);
    }
}
inline void init(Field* phi, Field* wa, Field* ua, Field* va, Field* r1, Field* r2, 
Field* Sa, Field* single1a, Field* single2a, Field* NL1a, Field* crossa, 
float lambda, float cn, float Pe){
    Mesh* mesh = r1->mesh;
...
// the intermediate components p (p11, p12, p21)
    ...
    coord(*mesh);
    init(phi, wa, ua, va, r1, r2, Sa, single1a, single2a, nl1a, crossa, lambda, cn, Pe);
    ...
    field_visual(crossa, "crossa.csv");
