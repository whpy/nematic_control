...
void wlin_func(float* IFwh, float* IFw, float* k_squared, float Re, float cf, float dt, int Nxh, int Ny)
{
    ...
    int index = j*Nxh + i;
    if(i<Nxh && j<Ny){
        IFwh[index] = exp((-1.0*k_squared[index])*dt/2);
        IFw[index] = exp((-1.0*k_squared[index])*dt);
    }
}
/******************************************************************************* 
 * visualization term                                                          * 
...
// in this unit, we test the performance on solving the linear problem involving
// spatial derivatives, i.e. the equation dw/dt = laplace(w), the exact solution
// should be exp(-t)*(cos(x)+sin(y)) where cos(x)+sin(y) is the initial condition. 
int main(){
